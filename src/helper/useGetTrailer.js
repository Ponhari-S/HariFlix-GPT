...
const useGetTrailer = (title) => {
    ...
    useEffect(()=>{
        ...
        async function getTrailer(title) {
            const data= await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(title + " official trailer")}&type=video&maxResults=1&key=${YT_API}`);
