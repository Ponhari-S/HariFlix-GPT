...
function Header() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        dispatch(setUser({ name: currentUser.displayName, profile: currentUser.photoURL }));
        navigate("/browse");
      } else {
        dispatch(removeUser());
        navigate("/");
  ...
  return (
    <div className="absolute w-full flex flex-col items-center md:flex-row md:justify-between md:items-center bg-gradient-to-b from-black to-transparent z-10 gap-3 md:gap-0 pb-3 md:pb-0">
      ...
      <svg
        ...
        aria-hidden="true"
        role="img"
        className="w-20 m-2 md:w-28 md:m-4 fill-red-600 flex-shrink-0"
      >
        <g>
      ...
      {userData.name != null && (
        <div className="flex flex-row flex-wrap justify-center md:justify-start gap-3 items-center px-3 md:px-0 md:pr-5">
          {gptTrue && (
            <select
              value={lang}
              onChange={(e) => dispatch(toggleLang(e.target.value))}
              className="bg-transparent text-white text-xs md:text-base border border-gray-600 cursor-pointer outline-none hover:border-red-600 transition px-1 py-0.5 md:px-2 md:py-1 rounded"
            >
          ...
          <button
            className="px-2 h-7 md:px-2 md:h-8 bg-red-600 text-white text-xs md:text-base rounded-md hover:bg-red-700 transition whitespace-nowrap"
            onClick={() => handleGPT()}
          >
          ...
          <img
            ...
            alt="Logo"
          ></img>
          <p className="font-bold text-red-800 text-xs md:text-base">
            {userData.name}
          </p>
          <button
